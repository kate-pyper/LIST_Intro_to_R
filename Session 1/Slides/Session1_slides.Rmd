...
Since you are familiar with doing analysis in other software (and to ease you in) you'll start by completing the tasks set out in Excel.
...
Then select an appropriate number of CPUs and Memory - the default is usually fine unless your are dealing with large amounts of data. There are guides for selecting appropriate memory [in general](https://public-health-scotland.github.io/knowledge-base/docs/Posit%20Infrastructure?doc=Quick%20guide%20to%20your%20Posit%20Workbench%20session.md) and for [SMR data](https://public-health-scotland.github.io/knowledge-base/docs/Posit%20Infrastructure?doc=Memory%20Usage%20in%20SMR01.md). 

Unless you are using parallel processing, you do not need more than 1 CPU.
...
Functions will take the form `function_name(args)` and will usually be represented in text as `function_name()`.
...
```
The symbol `<-` is the *assignment operator* which assigns the object to a name within the environment. This is needed for you to reuse the data in further analysis. If not used, the data will just print to the console.
---
...
This is important because a lot of functions in R produce output based on a vector of inputs. 
...
- `Character`: Any text values, usually representing categorical data. Values are surrounded by quotation marks `""` or `''` (though double quotations are always preferred!) 
...
We can simply specify column names within `select()` to specify which columns we would like to select. We can use `:` to select columns within a range. We can either specify the columns that we want to keep, or remove the columns that we don't want to keep using `!`. 
...
Each time you summarise, the last level of the grouping structure will be removed - so our data frame is no longer grouped by `age` in this case.
...
## Example: Datazone Populations to Locality Populations

Sometimes we may have data at a lower level than we need, and we want to aggregate it up. Usually this means using a lookup table which we join to the data to give use the higher level groups which we can then summarise by.

```{r}
# Read in the data
dz_pop <- read_rds("/conf/LIST_analytics/West Hub/03 - Training/Posit Training Cross Team/Introduction to R/Session 1/Data/dz_pops.rds")
lookup <- read_csv("/conf/linkage/output/lookups/Unicode/Geography/HSCP Locality/HSCP Localities_DZ11_Lookup_20230804.csv")

locality_pop <- dz_pop %>% 
  # Filter out correct year's data
  filter(year == max(year)) %>%
  # Join data set with lookup file: because of pipe this is equivalent to left_join(df1, lookup) 
  # where df1 is the filtered version of dz_pop
  left_join(lookup) %>% 
  # Group by locality
  group_by(hscp_locality) %>%
  # Calculate the total population
  summarise(pop = sum(pop))
# join population data and lookup data
