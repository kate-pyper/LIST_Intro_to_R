...
More likely we would want to *group by* some variable(s) first to calculate grouped summaries. To calculated the total male and female population within each HSCP:

```{r}
populations_summarised_group <- populations_selected %>% 
  # hscps are the same grouping, but we want to keep both columns
  group_by(hscp2019, hscp2019name, sex_name) %>% 
...
However if we use sum within the `mutate()` function we get some differences:
